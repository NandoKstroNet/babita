...
/*
 *	WPS CLASS é uma classe que acessa / manipula todo o conteúdo de um CMS WordPress
 *	COMPATIVEL COM WORDPRESS 4.0 OU SUPERIOR
 *
 *	Possibilidades:
 *
 *	1 - Criar diversos sistes, blogs, sistemas distintos alimentados por um unico banco / gerenciador;
 *	2 - Usar qualquer layout para criar um blog / site bem rápido sem a necessidade de tranforma-lo em um theme WordPress
 *	3 - Criar uma API e exportar posts / caregorys / pages / authors / etc em JSON ou XML
 *	4 - Exibir posts do wordpress em qualquer site / blog / sistema mesmo sem o meso ser WordPress
 *	5 - Etc. As possibilidades são muitas, bastando apenas imaginar e implementar / tentar.
 *
 *	AUTHOR: FÁBIO ASSUNÇÃO DA SILVA
 *	URI: http://fabioassuncao.com.br
 *	MAIL: fabio@fabioassuncao.com.br
 *	DATE: 23/10/2014
 */
...
class Wordpress extends Model {	
	...
	public function paginacao($total, $quantidade = null, $url = null){
		...
		$voltar = (Router::position('C') <= 1) ? 1 : Router::position('C') - 1;
		$avancar = (Router::position('C') == $num_paginas) ? $num_paginas : Router::position('C') + 1;
	
		$list = "<ul class='pagination'>";
		$list .= "<li><a href='{$url}/{$voltar}'>&laquo; Voltar</a></li>";
	
		for($i = 1; $i < $num_paginas + 1; $i++) {
			$active = ($i == Router::position('C')) ? 'active' : '';
